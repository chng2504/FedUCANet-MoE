...
def FedAvg(weights: List[Dict[str, torch.Tensor]], alpha: List[float] ) -> Dict[str, torch.Tensor]:
    """
    Federated Averaging
    weights: List[Dict[str, torch.Tensor]]: list of model weights, representing the weights of each client
    alpha: List[float]: list of alpha, representing the importance of each client
    """
    assert len(weights) == len(alpha)

    avg_weights = {}
    for key in weights[0].keys():
        avg_weights[key] = torch.sum(torch.stack([alpha[i] * weights[i][key] for i in range(len(weights))]), dim=0)
